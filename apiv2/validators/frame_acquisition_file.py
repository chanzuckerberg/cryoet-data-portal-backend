...
class FrameAcquisitionFileCreateInputValidator(BaseModel):
    ...
    s3_mdoc_path: Annotated[ str, StringConstraints(strip_whitespace=True)]
    https_mdoc_path: Annotated[ str, StringConstraints(strip_whitespace=True)]
    id: Annotated[ int, Field()]
class FrameAcquisitionFileUpdateInputValidator(BaseModel):
    # Pydantic stuff
    model_config = ConfigDict(from_attributes=True)
    run_id: Annotated[ uuid.UUID | None, Field()]
    s3_mdoc_path: Annotated[ str | None, StringConstraints(strip_whitespace=True)]
    https_mdoc_path: Annotated[ str | None, StringConstraints(strip_whitespace=True)]
