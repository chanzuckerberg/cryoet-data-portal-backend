...
class TomogramItem(ItemDBImporter):
    ...
    def load_computed_fields(self):
        ...
        extra_data = {
            ...
            "fiducial_alignment_status": normalize_fiducial_alignment(self.input_data.get("fiducial_alignment_status")),
            ...
            "s3_omezarr_dir": os.path.join(s3_prefix, s3_prefix, self.input_data["omezarr_dir"]),
            "https_omezarr_dir": os.path.join(https_prefix, s3_prefix, self.input_data["omezarr_dir"]),
            "s3_mrc_file": os.path.join(s3_prefix, s3_prefix, self.input_data["mrc_files"][0]),
            "https_mrc_file": os.path.join(https_prefix, s3_prefix, self.input_data["mrc_files"][0]),
        ...
        if not self.input_data.get("deposition_date"):
            deposition = self.config.deposition_map[self.input_data["deposition_id"]]
        ...
        deposition = get_deposition(self.config, self.input_data.get("deposition_id"))
...
class TomogramImporter(IntegratedDBImporter):
    ...
    clean_up_siblings = False # TODO should be true
    ...
    def get_filters(self) -> dict[str, Any]:
        return {"voxel_spacing_id": self.voxel_spacing.id}
