...
- They rely on the `sources` section of the entity's configuration.
...
Some of the commonly used finders are:
...
**Note:** Some entities like the `AlignmentImporters`, `AnnotationImporters` and, `VoxelSpacingImporters` have a more complex structure and require the outcome of the files found in the source to be handled differently than the other importers.
