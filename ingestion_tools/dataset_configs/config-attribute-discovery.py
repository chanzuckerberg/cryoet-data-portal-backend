...
def recursive_dict_update(current_entries, new_entries):
    for key, new_value in new_entries.items():
        ...
        if isinstance(new_value, dict) and (isinstance(current_entries.get(key), dict) or current_entries.get(key) is None):
            current_entries[key] = recursive_dict_update(current_entries.get(key, {}), new_value)
        # List and list situation
        elif isinstance(new_value, list) and (isinstance(current_entries.get(key), list) or current_entries.get(key) is None):
        ...
        # List and non-list
            ...
            elif len(current_entries.get(key)) == 1 and not isinstance(new_value, list) and not isinstance(new_value, dict):
                pass
            else:
                print("list conflict")
                print("Key: ", key)
...
def main():
    all_files = [os.path.join(directory_path, file) for directory_path, _, filename in os.walk(os.path.expanduser("./")) for file in filename]
