...
linkml_meta = LinkMLMeta(
    {
        ...
        "types": {
            ...
            "CDPO_ID": {
                "base": "str",
                "description": "A CryoET Data Portal Ontology " "identifier",
                "from_schema": "metadata",
                "name": "CDPO_ID",
                "pattern": "^CDPO:[0-9]{7}$",
            },
...
class AnnotationObject(ConfiguredBaseModel):
    ...
    def pattern_id(cls, v):
        pattern = re.compile(
            r"(^GO:[0-9]{7}$)|(^UniProtKB:(?:[OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z][0-9](?:[A-Z][A-Z0-9]{2}[0-9]){1,2})$)|(^UBERON:[0-9]{7}$)|(^CHEBI:[0-9]+$)|(^CDPO:[0-9]{7}$)"
        )
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
...
class AnnotationSemanticSegmentationMaskFile(AnnotationSourceFile):
    ...
    rescale: Optional[bool] = Field(
        ...
        json_schema_extra={
            "linkml_meta": {
                "domain_of": ["AnnotationSemanticSegmentationMaskFile", "AnnotationInstanceSegmentationMaskFile"],
...
class Annotation(AuthoredEntity, DateStampedEntity):
    ...
    annotation_ingest_id: str = Field(
        default=...,
        description="""A unique identifier used during ingestion to identify this annotation.""",
